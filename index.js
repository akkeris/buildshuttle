...
async function kubernetesLogsByPod(app_name, app_uuid, build_number) {
  return kube.logs(calcPodNameFromApp(app_name, app_uuid, build_number), 'akkeris-system');
}
...
async function runWorkerViaKubernetes(docker_build_image, logs, app_name, app_uuid, build_number, payload, callback, callback_auth) {
  ...
  try {
    const res = await kube.run(
      calcPodNameFromApp(app_name, app_uuid, build_number),
      'akkeris-system',
      'akkeris',
      docker_build_image,
      ['node', 'worker.js'],
...
async function runWorkerViaDocker(docker_build_image, logs, app_name, app_uuid, build_number, payload, callback, callback_auth) {
  ...
  const cenv = Object.keys(process.env).map((x) => `${x}=${process.env[x]}`)
    .concat([
      `PAYLOAD=${Buffer.from(typeof payload === 'string' ? payload : JSON.stringify(payload), 'utf8').toString('base64')}`,
    ]);
  const env = {
  ...
  return docker.run(docker_build_image, ['node', 'worker.js'], logs, env, async (err, data, container) => {
    try {
      if (err) {
        common.log(`Running worker returned with an error: ${err.message}\n${err.stack}`);
        return;
      }
      try {
        await container.remove({ v: true, link: true, force: true });
...
async function createBuild(req, res) {
  ...
  if (
    ...
    || !req.body.space
    || !req.body.app_uuid
    || !req.body.gm_registry_host
    || !req.body.gm_registry_repo
    || !req.body.build_number
    || !req.body.build_uuid) {
    res.status(400).send({ status: 'Bad Request - missing fields.' });
    return;
  }
  if (!(/([a-f\d]{8}(-[a-f\d]{4}){3}-[a-f\d]{12}?)/i).test(req.body.build_uuid)) {
    res.status(400).send({ status: 'Bad Request - build uuid is invalid.' });
    return;
  }
  const logs = new Logs(
    req.body.kafka_hosts,
    req.body.app,
    req.body.app_uuid,
    req.body.space,
    req.body.build_uuid,
...
async function getBuild(req, res) {
  try {
    ...
    const stream = await common.getObject(req.params.build);
    stream.on('error', (err) => {
...
async function stopBuild(req, res) {
  try {
    res.send({ status: 'ok' });
    if (process.env.USE_KUBERNETES === 'true') {
      const awkwardAppAndUUIDName = req.params.app_id.split('-');
      // TODO: stop this insanity with app, app uuid, build number..
      await stopKubernetesBuildByPod(
        awkwardAppAndUUIDName[0],
        awkwardAppAndUUIDName.slice(1).join('-'),
        req.params.number,
      );
    } else {
...
async function getBuildLogs(req, res) {
  try {
    ...
    if (process.env.TEST_MODE === 'true' && process.env.USE_KUBERNETES === 'true') {
      const awkwardAppAndUUIDName = req.params.app_id.split('-');
      const logline = await kubernetesLogsByPod(
        awkwardAppAndUUIDName[0],
        awkwardAppAndUUIDName.slice(1).join('-'),
        req.params.number,
      );
    ...
    const stream = await common.getObject(`${req.params.app_id}-${req.params.number}.logs`);
    stream.on('error', (err) => {
      if (
        err.message
        && err.message.indexOf('no such file') === -1
        && err.message.indexOf('specified key does not exist') === -1) {
