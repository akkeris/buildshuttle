...
function valid_response(code, res, message, headers) {
  res.writeHead(code, headers || common_headers());
    ...
    var req = client.request(options, (res) => {
      ...
      buffer_cb(res, (res_data) => {
        ...
        if( (res.statusCode < 200 || res.statusCode > 299) && res.statusCode !== 302 ) {
          callback({code:res.statusCode, message:(Buffer.isBuffer(res_data) ? res_data.toString('utf8') : res_data), headers:res.headers}, null);
        } else {
